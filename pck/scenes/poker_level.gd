...
func _on_server_respond(respond):
	if respond.head == "shankoemee":
		if respond.body.subhead == "new player":
			match (respond.body.status):
				"ok":
					$"/root/ws".gameState.game = "shankoemee"
					$"/root/ws".gameState.index = respond.body.index
					$"/root/ws".gameState.roomNo = respond.body.roomNo
				...
				"too much balance":
					$AlertBox._show("Too much balance")
				"player already exist":
					$AlertBox._show("Player already exist")
				"not active":
					$AlertBox._show("Poker server in maintenance please come back later")
...
func _on_level_pressed(level):
	...
	var url = $"/root/Config".config.account_url + "poker13"
	var http = HTTPRequest.new()
	add_child(http)
...
func _level_selected(result, response_code, headers, body):
	...
	if res.status == "ok":
		...
		get_tree().change_scene("res://pck/scenes/poker_13_game.tscn")
